...
## Submitted implementation quickstart

This repository contains the read-only TigerGraph fraud-investigation pilot and the 20 validated benchmark answers in `cases/`. The live graph uses the isolated `HHGOAFraudGraph` schema and restricted installed-query access. Credentials and the two oversized source CSVs are intentionally kept out of the public repository; place them locally before running ingestion.

1. Create a Python 3.11 environment and install the project with `pip install -e '.[dev]'`.
2. Copy `.env.example` to `.env`, then set the TigerGraph Savanna endpoint/secret. Set `HHGOA_GRAPH_BACKEND=tigergraph` and keep `HHGOA_READ_ONLY=true`.
3. Install the graph assets with `hhgoa-tigergraph-admin --install`, load normalized data with `hhgoa-tigergraph-admin --load`, and start the restricted MCP server described in [`docs/SETUP.md`](docs/SETUP.md).
4. Start the API with `uvicorn app.main:app --host 127.0.0.1 --port 8000`. The React dashboard is in `frontend/` and builds with `npm ci && npm run build`.
5. Run `pytest` for the local policy, persistence, answer-contract, and integration-boundary checks. The committed `cases/HHG-001.json` through `cases/HHG-020.json` are ready for submission.

The pilot never executes card controls, customer communication, or regulatory filing. Those actions are represented as policy-governed recommendations and simulated evidence in the exported cases. See [`docs/IMPLEMENTATION_STATUS.md`](docs/IMPLEMENTATION_STATUS.md) for the verified Savanna counts and known limits.
